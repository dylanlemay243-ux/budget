...
// User-facing presets. Each maps to a base frequency + an interval multiplier,
// so "Every 2 weeks" is just weekly×2, "Quarterly" is monthly×3, etc.
interface RecurPreset {id: string; label: string; freq: RecurrenceFreq; interval: number;}
const RECUR_PRESETS: RecurPreset[] = [
  {id: 'none', label: 'One-time', freq: 'none', interval: 1},
  {id: 'daily', label: 'Daily', freq: 'daily', interval: 1},
  {id: 'weekly', label: 'Weekly', freq: 'weekly', interval: 1},
  {id: 'biweekly', label: 'Every 2 weeks', freq: 'weekly', interval: 2},
  {id: 'monthly', label: 'Monthly', freq: 'monthly', interval: 1},
  {id: 'quarterly', label: 'Quarterly', freq: 'monthly', interval: 3},
  {id: 'yearly', label: 'Yearly', freq: 'yearly', interval: 1},
];

function recurrenceLabel(freq?: RecurrenceFreq, interval = 1): string {
  if (!freq || freq === 'none') return '';
  const p = RECUR_PRESETS.find(x => x.freq === freq && x.interval === interval);
  if (p) return p.label;
  const unit = freq === 'daily' ? 'day' : freq === 'weekly' ? 'week' : freq === 'monthly' ? 'month' : 'year';
  return `Every ${interval} ${unit}s`;
}

// Unique-ish id for newly created (or split) series.
...
interface Entry {
  ...
  interval?: number;             // repeat every N units of `recurrence`; defaults to 1
  recurrenceEnd?: string;        // optional YYYY-MM-DD; recurrence stops on/before this date
  exceptions?: string[];         // specific occurrence dates to skip (deleted single events)
...
function occurrenceDate(freq: RecurrenceFreq, anchor: string, k: number, interval = 1): string {
  const step = k * (interval || 1);
  switch (freq) {
    case 'daily': return addDaysISO(anchor, step);
    case 'weekly': return addDaysISO(anchor, 7 * step);
    case 'monthly': return addMonthsClamped(anchor, step);
    case 'yearly': return addYearsClamped(anchor, step);
...
// First occurrence of a series strictly after `afterDate` (respecting exceptions
// and the series end). Returns null if none remain.
function nextOccurrenceAfter(entry: Entry, afterDate: string): string | null {
  const freq = entry.recurrence ?? 'none';
  if (freq === 'none') return entry.date > afterDate ? entry.date : null;
  const interval = entry.interval ?? 1;
  const exc = entry.exceptions && entry.exceptions.length ? new Set(entry.exceptions) : null;
  for (let k = 0; k < MAX_OCCURRENCES; k++) {
    const d = occurrenceDate(freq, entry.date, k, interval);
    if (entry.recurrenceEnd && d > entry.recurrenceEnd) return null;
    if (d > afterDate && !(exc && exc.has(d))) return d;
  }
  return null;
}
...
function occurrencesOf(entry: Entry, winStart: string, winEnd: string): Entry[] {
  ...
  const exc = entry.exceptions && entry.exceptions.length ? new Set(entry.exceptions) : null;
  if (freq === 'none') {
    return entry.date >= winStart && entry.date <= winEnd && !(exc && exc.has(entry.date))
  ...
  for (let k = 0; k < MAX_OCCURRENCES; k++) {
    const d = occurrenceDate(freq, entry.date, k, interval);
    if (d > hardEnd) break;
    if (d >= winStart && !(exc && exc.has(d))) out.push(makeOccurrence(entry, d));
  }
  return out;
}
...
export default function App() {
  ...
  const [editOccurrence, setEditOccurrence] = useState<Entry | null>(null);
  const [catPickerOpen, setCatPickerOpen] = useState(false);
  const [calMonth, setCalMonth] = useState(() => {
    const d = new Date();
    return {yyyy: d.getFullYear(), mm: d.getMonth() + 1};
  });
  // Generic "apply to which events?" chooser (works on Android, unlike a >3-button Alert)
  ...
  const openEdit = useCallback((occurrence: Entry) => {
    // Load the underlying stored series, but remember which occurrence was tapped
    // so "this & following" / "only this" edits know where to split.
    ...
    setFormInterval(stored.interval ?? 1);
    setFormRecurrenceEnd(stored.recurrenceEnd ?? '');
    setModalVisible(true);
  }, [entries]);

  // Apply edited form values to the whole series (or a plain entry) in place.
  const applyEditAll = useCallback((vals: Partial<Entry>) => {
    if (!editEntry) return;
    setEntries(prev => prev.map(e => (e.id === editEntry.id ? {...e, ...vals} : e)));
  }, [editEntry]);

  // Split the series at the tapped occurrence: original ends the day before, a new
  // series carrying the edited values starts on that date. Used for raises, etc.
  const applyEditThisAndFollowing = useCallback((vals: Partial<Entry>) => {
    if (!editEntry || !editOccurrence) return;
    const splitDate = editOccurrence.date;
    setEntries(prev => {
      const orig = prev.find(e => e.id === editEntry.id);
      if (!orig) return prev;
      const dayBefore = addDaysISO(splitDate, -1);
      const origExc = (orig.exceptions ?? []).filter(d => d < splitDate);
      const newExc = (orig.exceptions ?? []).filter(d => d >= splitDate);
      const newSeries: Entry = {
        id: genId(),
        type: vals.type ?? orig.type,
        amount: vals.amount ?? orig.amount,
        label: (vals.label ?? orig.label) as string,
        category: (vals.category ?? orig.category) as Category,
        date: splitDate,
        timestamp: Date.now(),
        recurrence: vals.recurrence ?? orig.recurrence,
        interval: vals.interval ?? orig.interval ?? 1,
        recurrenceEnd: orig.recurrenceEnd, // inherit the overall end of the series
        exceptions: newExc.length ? newExc : undefined,
      };
      const truncated: Entry = {...orig, recurrenceEnd: orig.recurrenceEnd && orig.recurrenceEnd < dayBefore ? orig.recurrenceEnd : dayBefore, exceptions: origExc.length ? origExc : undefined};
      let result = prev.map(e => (e.id === orig.id ? truncated : e));
      // If nothing remains before the split, drop the original head entirely.
      if (truncated.date > (truncated.recurrenceEnd as string)) result = result.filter(e => e.id !== orig.id);
      return [newSeries, ...result];
    });
  }, [editEntry, editOccurrence]);

  // Turn just the tapped date into a one-off override: skip it in the series and
  // add a standalone entry with the edited values.
  const applyEditOnlyThis = useCallback((vals: Partial<Entry>) => {
    if (!editEntry || !editOccurrence) return;
    const d = editOccurrence.date;
    setEntries(prev => {
      const orig = prev.find(e => e.id === editEntry.id);
      if (!orig) return prev;
      const single: Entry = {
      ...
      const withExc: Entry = {...orig, exceptions: [...(orig.exceptions ?? []), d]};
      return [single, ...prev.map(e => (e.id === orig.id ? withExc : e))];
    });
  }, [editEntry, editOccurrence]);
  ...
  const handleSave = useCallback(() => {
    ...
    const vals: Partial<Entry> = {
      type: formType, amount, label: formLabel.trim(), category: formCategory,
      date: formDate, recurrence: formRecurrence,
      interval: formRecurrence === 'none' ? 1 : formInterval, recurrenceEnd,
    };

    if (editEntry) {
      const wasRecurring = !!editEntry.recurrence && editEntry.recurrence !== 'none';
      const onMiddleOrLater = editOccurrence && editOccurrence.date > editEntry.date;
      if (wasRecurring && editOccurrence) {
        // Offer scope only when a split is meaningful (not on the very first date).
        const options = [
          {label: 'Only this event', onPress: () => {applyEditOnlyThis(vals); setModalVisible(false);}},
          ...(onMiddleOrLater ? [{label: 'This & all following events', onPress: () => {applyEditThisAndFollowing(vals); setModalVisible(false);}}] : []),
          {label: 'All events in the series', onPress: () => {applyEditAll(vals); setModalVisible(false);}},
        ];
        setScopePrompt({title: 'Update recurring entry', message: `Editing the occurrence on ${editOccurrence.date}. Apply this change to…`, options});
        return;
      }
      applyEditAll(vals);
    } else {
      setEntries(prev => [{id: genId(), timestamp: Date.now(), ...(vals as Entry)}, ...prev]);
    }
    setModalVisible(false);
  }, [editEntry, editOccurrence, formType, formAmount, formLabel, formCategory, formDate, formRecurrence, formInterval, formRecurrenceEnd, applyEditAll, applyEditThisAndFollowing, applyEditOnlyThis]);

  const handleDelete = useCallback((occurrence: Entry) => {
    const seriesId = occurrence.seriesId ?? occurrence.id;
    const d = occurrence.date;
    const removeSeries = () => setEntries(prev => prev.filter(e => e.id !== seriesId));

    if (!occurrence.isRecurring) {
      Alert.alert('Delete entry', 'Remove this entry?', [
        {text: 'Cancel', style: 'cancel'},
        {text: 'Delete', style: 'destructive', onPress: removeSeries},
      ]);
      return;
    }

    // Delete just this date (add an exception).
    const deleteOnlyThis = () => setEntries(prev => prev.map(e =>
      e.id === seriesId ? {...e, exceptions: [...(e.exceptions ?? []), d]} : e));

    // Delete this date and everything after (truncate the tail).
    const deleteThisAndAfter = () => setEntries(prev => {
      const s = prev.find(e => e.id === seriesId);
      if (!s) return prev;
      const dayBefore = addDaysISO(d, -1);
      if (s.date > dayBefore) return prev.filter(e => e.id !== seriesId);
      const newEnd = s.recurrenceEnd && s.recurrenceEnd < dayBefore ? s.recurrenceEnd : dayBefore;
      const exc = (s.exceptions ?? []).filter(x => x <= dayBefore);
      return prev.map(e => (e.id === seriesId ? {...s, recurrenceEnd: newEnd, exceptions: exc.length ? exc : undefined} : e));
    });

    // Delete this date and everything before (advance the anchor forward).
    const deleteThisAndBefore = () => setEntries(prev => {
      const s = prev.find(e => e.id === seriesId);
      if (!s) return prev;
      const nextAnchor = nextOccurrenceAfter(s, d);
      if (!nextAnchor) return prev.filter(e => e.id !== seriesId);
      const exc = (s.exceptions ?? []).filter(x => x >= nextAnchor);
      return prev.map(e => (e.id === seriesId ? {...s, date: nextAnchor, exceptions: exc.length ? exc : undefined} : e));
    });

    setScopePrompt({
      title: 'Delete recurring entry',
      message: `This occurrence is on ${d}. What would you like to delete?`,
      options: [
        {label: 'This event only', onPress: deleteOnlyThis},
        {label: 'This & all following events', onPress: deleteThisAndAfter},
        {label: 'This & all previous events', onPress: deleteThisAndBefore},
        {label: 'Entire series', destructive: true, onPress: removeSeries},
      ],
    });
  }, []);
                  ...
                  <View style={s.entryLabelRow}>
                    ...
                    {entry.isRecurring && (
                      <View style={s.recurBadge}>
                        <Text style={s.recurBadgeText}>🔁 {recurrenceLabel(entry.recurrence, entry.interval)}</Text>
                      </View>
                    )}
                  </View>
  ...
  return (
    <SafeAreaView style={s.safe}>
          ...
          <View style={s.modalCard}>
            ...
            <View style={s.recurRow}>
              {RECUR_PRESETS.map(opt => {
                const active = formRecurrence === opt.freq && (formRecurrence === 'none' || formInterval === opt.interval);
                return (
                  <TouchableOpacity
                    key={opt.id}
                    style={[s.recurChip, active && s.recurChipActive]}
                    onPress={() => {setFormRecurrence(opt.freq); setFormInterval(opt.interval);}}>
                    <Text style={[s.recurChipText, active && s.recurChipTextActive]}>{opt.label}</Text>
                  </TouchableOpacity>
                );
              })}
            </View>
      ...
      {/* Scope chooser (edit/delete a recurring series) */}
      <Modal visible={!!scopePrompt} animationType="fade" transparent onRequestClose={() => setScopePrompt(null)}>
        <TouchableOpacity style={s.scopeOverlay} activeOpacity={1} onPress={() => setScopePrompt(null)}>
          <View style={s.scopeSheet}>
            <Text style={s.scopeTitle}>{scopePrompt?.title}</Text>
            {!!scopePrompt?.message && <Text style={s.scopeMsg}>{scopePrompt.message}</Text>}
            {scopePrompt?.options.map((opt, i) => (
              <TouchableOpacity
                key={i}
                style={s.scopeOption}
                onPress={() => {const fn = opt.onPress; setScopePrompt(null); fn();}}>
                <Text style={[s.scopeOptionText, opt.destructive && {color: '#F44336'}]}>{opt.label}</Text>
              </TouchableOpacity>
            ))}
            <TouchableOpacity style={[s.scopeOption, s.scopeCancel]} onPress={() => setScopePrompt(null)}>
              <Text style={s.scopeCancelText}>Cancel</Text>
            </TouchableOpacity>
          </View>
        </TouchableOpacity>
      </Modal>
...
const s = StyleSheet.create({
  ...
  scopeOverlay: {flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', alignItems: 'center', padding: 20},
  scopeSheet: {backgroundColor: '#1A2A3A', borderRadius: 16, padding: 16, width: '86%', maxWidth: 380},
  scopeTitle: {color: '#E0EAF4', fontSize: 17, fontWeight: '800', marginBottom: 4},
  scopeMsg: {color: '#8BAFC9', fontSize: 13, marginBottom: 12, lineHeight: 18},
  scopeOption: {paddingVertical: 13, paddingHorizontal: 14, borderRadius: 10, backgroundColor: '#0D1B2A', marginTop: 8},
  scopeOptionText: {color: '#E0EAF4', fontSize: 15, fontWeight: '600', textAlign: 'center'},
  scopeCancel: {backgroundColor: 'transparent', marginTop: 12},
  scopeCancelText: {color: '#8BAFC9', fontSize: 15, fontWeight: '600', textAlign: 'center'},
